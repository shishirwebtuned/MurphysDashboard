  ...
  async (
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/assign_details/${client_id}/${service_catalog_id}`,
      );
      ...
      return rejectWithValue(
        error?.response?.data?.message ||
          error?.message ||
          "Failed to fetch assign details",
      );
    }
  },
);
  ...
  async (payload: { id: string | null }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message ||
          error.message ||
          "Failed to delete assigned service",
      );
    }
  },
);
...
export const addRenewalDate = createAsyncThunk(
  ...
  async (
    ...
    try {
      const { id, renewal_date, renewal_label, renewal_price, renewal_id } =
        payload;
      const body: any = {
...
const assignSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        deleteAssignedService.fulfilled,
        (state, action: PayloadAction<any>) => {
          state.loading = false;
      ...
      .addCase(
        updateAssignedService.fulfilled,
        (state, action: PayloadAction<any>) => {
          state.loading = false;
