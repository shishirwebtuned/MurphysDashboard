...
export const createProfile = async (
  ...
  try {
    ...
    res
      .status(201)
      .json({ data: profile, message: "Profile created successfully" });
  } catch (error) {
...
export const toggleUserPermission = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `Permission toggle: user=${userId} permission=${permission} action=${permissionIndex > -1 ? "removed" : "added"}`,
    );
...
export const getUserPermissions = async (req: Request, res: Response) => {
  try {
    ...
    const allPermissions = [
      ...new Set([...rolePermissions, ...(profile.permissions || [])]),
    ];
    res.status(200).json({
      data: {
        userId: profile._id,
...
export const deleteProfile = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      data: profile,
      message:
        "Profile and associated authentication data deleted successfully",
    });
  } catch (error) {
    res.status(400).json({ message: (error as Error).message });
  }
