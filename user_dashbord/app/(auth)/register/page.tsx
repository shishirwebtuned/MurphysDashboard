...
// Copy all animation constants, components, and utilities from login page
...
export default function RegisterPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setMood("sad");
      setServerError(err?.response?.data?.message ?? err?.message ?? 'Failed to send verification email');
    } finally {
      setLoading(false);
    }
  };
